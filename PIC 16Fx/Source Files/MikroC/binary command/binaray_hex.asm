;  ASM code generated by mikroVirtualMachine for PIC - V. 8.2.0.0
;  Date/Time: 9/6/2016 9:21:39 AM
;  Info: http://www.mikroe.com


; ADDRESS	OPCODE	ASM
; ----------------------------------------------
$0000	$2804			GOTO	_main
$0004	$	_main:
;binaray_hex.c,1 :: 		void main()
;binaray_hex.c,3 :: 		TRISB=0;
$0004	$1303			BCF	STATUS, RP1
$0005	$1683			BSF	STATUS, RP0
$0006	$0186			CLRF	TRISB, 1
;binaray_hex.c,4 :: 		PORTB=0b11001100;
$0007	$30CC			MOVLW	204
$0008	$1283			BCF	STATUS, RP0
$0009	$0086			MOVWF	PORTB
;binaray_hex.c,5 :: 		DELAY_ms(1000);
$000A	$300B			MOVLW	11
$000B	$00FC			MOVWF	STACK_12
$000C	$30FF			MOVLW	255
$000D	$00FB			MOVWF	STACK_11
$000E	$30FF			MOVLW	255
$000F	$00FA			MOVWF	STACK_10
$0010	$0BFC			DECFSZ	STACK_12, F
$0011	$2813			GOTO	$+2
$0012	$281A			GOTO	$+8
$0013	$0BFB			DECFSZ	STACK_11, F
$0014	$2816			GOTO	$+2
$0015	$2819			GOTO	$+4
$0016	$0BFA			DECFSZ	STACK_10, F
$0017	$2816			GOTO	$-1
$0018	$2813			GOTO	$-5
$0019	$2810			GOTO	$-9
$001A	$3033			MOVLW	51
$001B	$00FB			MOVWF	STACK_11
$001C	$30FF			MOVLW	255
$001D	$00FA			MOVWF	STACK_10
$001E	$0BFB			DECFSZ	STACK_11, F
$001F	$2821			GOTO	$+2
$0020	$2824			GOTO	$+4
$0021	$0BFA			DECFSZ	STACK_10, F
$0022	$2821			GOTO	$-1
$0023	$281E			GOTO	$-5
$0024	$3088			MOVLW	136
$0025	$00FA			MOVWF	STACK_10
$0026	$0BFA			DECFSZ	STACK_10, F
$0027	$2826			GOTO	$-1
;binaray_hex.c,6 :: 		PORTB=0X00;
$0028	$0186			CLRF	PORTB, 1
;binaray_hex.c,7 :: 		DELAY_ms(1000);
$0029	$300B			MOVLW	11
$002A	$00FC			MOVWF	STACK_12
$002B	$30FF			MOVLW	255
$002C	$00FB			MOVWF	STACK_11
$002D	$30FF			MOVLW	255
$002E	$00FA			MOVWF	STACK_10
$002F	$0BFC			DECFSZ	STACK_12, F
$0030	$2832			GOTO	$+2
$0031	$2839			GOTO	$+8
$0032	$0BFB			DECFSZ	STACK_11, F
$0033	$2835			GOTO	$+2
$0034	$2838			GOTO	$+4
$0035	$0BFA			DECFSZ	STACK_10, F
$0036	$2835			GOTO	$-1
$0037	$2832			GOTO	$-5
$0038	$282F			GOTO	$-9
$0039	$3033			MOVLW	51
$003A	$00FB			MOVWF	STACK_11
$003B	$30FF			MOVLW	255
$003C	$00FA			MOVWF	STACK_10
$003D	$0BFB			DECFSZ	STACK_11, F
$003E	$2840			GOTO	$+2
$003F	$2843			GOTO	$+4
$0040	$0BFA			DECFSZ	STACK_10, F
$0041	$2840			GOTO	$-1
$0042	$283D			GOTO	$-5
$0043	$3088			MOVLW	136
$0044	$00FA			MOVWF	STACK_10
$0045	$0BFA			DECFSZ	STACK_10, F
$0046	$2845			GOTO	$-1
;binaray_hex.c,8 :: 		}
$0047	$2847			GOTO	$
